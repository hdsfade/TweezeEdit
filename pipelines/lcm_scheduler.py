...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int, lcm_origin_steps: int, device: Union[str, torch.device] = None):
        ...
        lcm_origin_timesteps = np.asarray(
            ...
            list(range(1, lcm_origin_steps + 1))) * c - 1
    ...
    def forward(
        ...
        if len(self.timesteps) > 1:
            ...
            sample_t = alpha_prod_t.sqrt() * sample + beta_prod_t.sqrt() * noise
        else:
            sample_t = sample
        return sample_t, noise
    ...
    def add_scale_noise(
        ...
        if len(self.timesteps) > 1:
            if noise is None:
                noise = torch.randn(sample.shape).to(sample.device)
            if opt:
                sample_t = sample + beta_prod_t.sqrt() * noise
            else:
                sample_t = sample - beta_prod_t.sqrt() * noise
        else:
            sample_t = sample
    ...
    def step_tweezeedit(
        ...
        beta_prod_t_prev = 1 - alpha_prod_t_prev
        if len(self.timesteps) > 1:
            z_inv = z_src + alpha_prod_t_prev.sqrt() * (z_tar_pred - z_src_pred)
        else:
            z_inv = z_tar_pred
        return z_inv
    ...
    def approx_rt(
        ...
        if len(self.timesteps) > 1:
            r_t = (alpha_prod_t.sqrt() - alpha_prod_t_prev.sqrt()) * \
                (z_src_pred - z_tar_pred) - beta_prod_t.sqrt() * noise_diff
        else:
            r_t = 0
    ...
    def original_rt(
        ...
        beta_t_prv = self.betas[prev_timestep]

        delta_t = (timestep - prev_timestep) / 2 / \
            self.config.num_train_timesteps
        derivative_alpha_prod_t = (
            alpha_prod_t - alpha_prod_t_prev) / (delta_t * 2)

        term1 = (-1 + derivative_alpha_prod_t * delta_t / alpha_prod_t/2)
        term2 = (-1 + derivative_alpha_prod_t *
                 delta_t / alpha_prod_t.sqrt()/2)
        term3 = (-alpha_prod_t.sqrt() + derivative_alpha_prod_t *
                 delta_t / alpha_prod_t.sqrt()/2)

        if len(self.timesteps) > 1:
            r_t = -2 * term1 * (alpha_prod_t.sqrt() * (z_src_pred - z_tar_pred) -
                                beta_prod_t.sqrt() * noise_diff - (z_src_pred - z_tar_pred) * term2)
        else:
            r_t = 0
        # normalization
        scale = (-alpha_prod_t.sqrt() + alpha_prod_t_prev.sqrt()) / \
            (-2 * term1 * term3)
        return r_t * scale * gamma
